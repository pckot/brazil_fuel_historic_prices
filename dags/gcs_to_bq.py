...
def gcs_to_bq_dag():
    ...
    def process_files(files_to_process, cluster_created):
        """Process files and submit jobs to Dataproc"""
    ...
    files_to_submit = process_files(files_to_process, create_cluster_decision)
    
    # Now create the submit job tasks dynamically
    # We need to use a slightly different approach due to how TaskFlow API works
    file_list = files_to_submit  # This will be a list of file names or empty
    
    # Keep track of submitted jobs for clean dependencies
    submitted_jobs = []
    
    # This is where we will create the dynamic tasks
    # Note that this happens at DAG definition time, not at runtime
    # We need to handle the case where file_list might be empty
    def create_submission_tasks():
        for i, file in enumerate(file_list):
            if isinstance(file, str):  # Make sure it's a string
                file_safe_name = file.replace(".", "_").replace("-", "_")
                ...
                submit_task = DataprocSubmitJobOperator(
                    task_id=task_id,
                    job=job_config,
                    region=GCP_REGION,
                    project_id=GCP_PROJECT_ID,
                ...
                # Record file as processed after job completion
                record_task = record_processed_file.override(task_id=f'record_{file_safe_name}')(file)
                
                # Set up dependencies
                create_cluster >> submit_task >> record_task >> delete_cluster
                
                # Add to list of submitted jobs
                submitted_jobs.append(submit_task)
    
    # Create the tasks
    create_submission_tasks()
